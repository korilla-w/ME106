...
lv_obj_t * screen_settings_wifi_manage_create(void)
{
    ...
    lv_obj_add_screen_create_event(wifi_manage_btn_return, LV_EVENT_CLICKED, screen_settings_create, LV_SCREEN_LOAD_ANIM_NONE, 0, 0);
    ...
    lv_anim_timeline_t ** at_array = lv_malloc(sizeof(lv_anim_timeline_t *) * _SCREEN_SETTINGS_WIFI_MANAGE_TIMELINE_CNT);
    at_array[SCREEN_SETTINGS_WIFI_MANAGE_TIMELINE_CONTENT_OPEN] = timeline_content_open_create(lv_obj_0);
    lv_obj_set_user_data(lv_obj_0, at_array);
    lv_obj_add_event_cb(lv_obj_0, free_timeline_event_cb, LV_EVENT_DELETE, at_array);

    lv_obj_add_play_timeline_event(lv_obj_0, LV_EVENT_SCREEN_LOADED, screen_settings_wifi_manage_get_timeline(lv_obj_0, SCREEN_SETTINGS_WIFI_MANAGE_TIMELINE_CONTENT_OPEN), 0, false);
...
{
    if (timeline_id >= _SCREEN_SETTINGS_WIFI_MANAGE_TIMELINE_CNT) {
        LV_LOG_WARN("screen_settings_wifi_manage has no timeline with %d ID", timeline_id);
        return NULL;
    }
...
static void free_timeline_event_cb(lv_event_t * e)
{
    ...
    uint32_t i;
    for(i = 0; i < _SCREEN_SETTINGS_WIFI_MANAGE_TIMELINE_CNT; i++) {
