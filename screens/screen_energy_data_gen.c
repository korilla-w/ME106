...
lv_obj_t * screen_energy_data_create(void)
{
    ...
    lv_obj_t * energy_header = lv_obj_create(page_content);
    lv_obj_set_name(energy_header, "energy_header");
    lv_obj_set_x(energy_header, 0);
    lv_obj_set_y(energy_header, 0);
    lv_obj_set_width(energy_header, 320);
    lv_obj_set_height(energy_header, 44);
    lv_obj_set_style_bg_opa(energy_header, 0, 0);
    lv_obj_set_style_border_width(energy_header, 0, 0);
    lv_obj_set_style_pad_all(energy_header, 0, 0);
    lv_obj_set_flag(energy_header, LV_OBJ_FLAG_SCROLLABLE, false);
    lv_obj_t * energy_brand_logo = lv_image_create(energy_header);
    lv_obj_set_name(energy_brand_logo, "energy_brand_logo");
    lv_image_set_src(energy_brand_logo, meter_brand_logo);
    lv_obj_set_x(energy_brand_logo, 20);
    lv_obj_set_y(energy_brand_logo, 17);

    lv_obj_t * lv_label_0 = lv_label_create(energy_header);
    ...
    lv_obj_add_screen_create_event(energy_btn_settings, LV_EVENT_CLICKED, screen_settings_create, LV_SCREEN_LOAD_ANIM_FADE_IN, 125, 0);
    ...
    lv_obj_add_screen_create_event(energy_btn_reserved, LV_EVENT_CLICKED, screen_function_menu_create, LV_SCREEN_LOAD_ANIM_FADE_IN, 125, 0);
    ...
    at_array[SCREEN_ENERGY_DATA_TIMELINE_MEASUREMENT_HEADER_OPEN] = timeline_measurement_header_open_create(lv_obj_0);
    lv_obj_set_user_data(lv_obj_0, at_array);
    lv_obj_add_event_cb(lv_obj_0, free_timeline_event_cb, LV_EVENT_DELETE, at_array);

    lv_obj_add_play_timeline_event(lv_obj_0, LV_EVENT_SCREEN_LOADED, screen_energy_data_get_timeline(lv_obj_0, SCREEN_ENERGY_DATA_TIMELINE_MEASUREMENT_HEADER_OPEN), 0, false);
...
static lv_anim_timeline_t * timeline_measurement_header_open_create(lv_obj_t * obj)
{
    ...
    lv_anim_set_var(&a, lv_obj_find_by_name(obj, "energy_header"));
